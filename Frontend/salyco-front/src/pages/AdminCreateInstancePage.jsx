...
export default function AdminCreateInstancePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [userRes, mattressData] = await Promise.all([
          api.get("/api/user/me/"),
          listMattresses(),
        ]);
        const list = Array.isArray(mattressData) ? mattressData : mattressData.results || [];
        setMattresses(list);
        setIsAdmin(userRes.data.is_staff === true);
      } catch {
        setIsAdmin(false);
      }
    })();
  ...
  if (isAdmin === null) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-wheat-50 pt-[var(--navbar-height)]">
      ...
      <div className="flex min-h-screen items-center justify-center bg-wheat-50 pt-[var(--navbar-height)]">
        <div className="overflow-hidden rounded-2xl border border-red-200 bg-white p-12 text-center shadow-sm">
          ...
          <p className="mt-2 font-persian text-sm text-[#000c3e]/60" dir="rtl">
            فقط مدیران سیستم به این صفحه دسترسی دارند.
          </p>
          <button
            onClick={() => navigate("/")}
            className="mt-6 inline-flex items-center gap-2 rounded-full border border-blue-400/20 bg-white px-6 py-3 font-persian font-medium text-[#000c3e] transition hover:bg-[#F5F7FA]"
          >
            بازگشت به صفحه اصلی
          </button>
        </div>
        ...
        {!result ? (
          <div className="overflow-hidden rounded-2xl border border-blue-400/15 bg-white p-8 shadow-sm">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {error && (
                <div className="flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 p-3" dir="rtl">
          ...
          <div className="overflow-hidden rounded-2xl border border-blue-400/15 bg-white p-8 shadow-sm">
            ...
            <div className="mt-8 space-y-4" dir="rtl">
              <div className="flex items-center justify-between rounded-xl border border-blue-400/10 bg-[#F5F7FA]/80 p-4">
                <span className="font-persian text-sm font-medium text-[#000c3e]/60">شماره سریال</span>
              ...
              {result.warranty_url && (
                <div className="flex items-center justify-between rounded-xl border border-blue-400/10 bg-[#F5F7FA]/80 p-4">
                  <span className="font-persian text-sm font-medium text-[#000c3e]/60">لینک گارانتی</span>
